...
**IP-04 still to do:** wire `ExamsScreen.jsx` to the store (persist created exams + attempts + immutable results; resume open attempts; validate restored identity before entering Attempt) and replace `examsModel.evaluateAttempt` (fabricated length/index grading) with the exam service, showing honest ungraded results when no provider is configured; route each miss's remediation into Study via the stable-id handoff (reuses the R-018 path already verified).

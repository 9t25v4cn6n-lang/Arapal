...
**IP-03 remaining (2/2):** wire the boundary into `submitSegment`/Study so a form-only surface check never yields a semantic "passed" — when no provider is configured, a submission is saved as an honest *attempt* (not a green pass), and with a provider the real grade drives pass/fail and the review outputs. The unconfigured/attempt path is browser-verifiable here; the real-grade path needs a BYO provider key.
